...
{#if board}
  <div class="mx-auto flex max-w-md flex-col gap-4 p-4">
    ...
    <p class="font-ui text-body-sm text-slate-gray">
      Lime cells arrived from the communal pool — authorship stays hidden until results.
    </p>

    <div class="grid grid-cols-5 gap-1.5">
      {#each board.cells as cell, i (i)}
        <div
          class="flex aspect-square items-center justify-center overflow-hidden rounded-[var(--radius-tag)] border border-near-black p-1 text-center font-ui text-[11px] leading-tight"
          class:bg-lime-spark={cell.fromPool}
