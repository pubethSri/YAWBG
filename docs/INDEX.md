...
| 11 | [`11-cross-game-portal.md`](11-cross-game-portal.md) | **Considered twice, declined twice — not a plan, and §7 is not a work queue.** ito vs YAWBG connection/session logic compared side by side; the three portal options (static page / shared room registry / full merge) and why the merge is not recommended; the shared-analytics envelope and admin-OIDC candidates (both still genuinely open) | Someone asks whether to combine the games or add a shared landing page — read the banner before re-opening it |
