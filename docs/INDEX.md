...
| 10 | [`10-highlight-reel.md`](10-highlight-reel.md) | **Built (M5.5).** The fourth results stage: rotating cards pairing a round's topic with every name proposed for it (withdrawn included), plus the **cheer** mechanic, its hidden tally, the v4 protocol shapes, a 25-step manual test and an implementation postscript | You're building M5.5, or you need to know why cheers aren't voting |
| 11 | [`11-cross-game-portal.md`](11-cross-game-portal.md) | **Picked up 2026-08-04 — superseded by doc 12, kept as the evidence behind it.** ito vs YAWBG connection/session logic compared side by side; the three portal options (static page / shared room registry / full merge) and why the merge is not recommended; the shared-analytics envelope and admin-OIDC candidates (both still open) | You want the reasoning *behind* the hub decision, or the two-codebase comparison. For what's being built, read doc 12 |
| 12 | [`12-hub-and-deployment.md`](12-hub-and-deployment.md) | **The plan of record for M6.** The **cruzhub** static landing page across three apps (YAWBG, *ito*, *Pastebin*); flat-subdomain topology and why not path-based; the edge Caddy stack; the auto-submitting deep-link contract and its five rules; the development story (and why an env check is the wrong shape); per-app work incl. Pastebin's hardening; the ordered cutover; every cross-repo conflict and its resolution | You're deploying anything, touching the hub, or handing work to the ito / Pastebin agents |
