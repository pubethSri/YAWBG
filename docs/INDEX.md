...
| 11 | [`11-cross-game-portal.md`](11-cross-game-portal.md) | **Assessment only, nothing decided.** ito vs YAWBG connection/session logic compared side by side; the three portal options (static page / shared room registry / full merge) and why the merge is not recommended; the shared-analytics envelope and admin-OIDC candidates; a scoped question list for each project's agent | Someone asks whether to combine the games, share a login/portal, or unify analytics across them |
